...
# 当日信号历史轻量缓存：Streamlit 秒级轮询时避免反复穿透 SQLite
_SIGNAL_HISTORY_CACHE: dict[str, tuple[float, list[dict[str, Any]]]] = {}
_SIGNAL_HISTORY_CACHE_TTL_SEC = max(
    1.0,
    float(os.environ.get("QUANT_SIGNAL_HISTORY_CACHE_SEC", "12")),
)
_ST_SESSION_SIGNAL_CACHE_KEY = "_quant_signal_history_cache"


def _signal_history_cache_key(
    stock_code: str,
...
def _streamlit_signal_cache_bucket() -> dict[str, dict[str, Any]] | None:
    """Streamlit 会话级缓存桶（非 UI 环境返回 None）。"""
    try:
        import streamlit as st

        bucket = st.session_state.get(_ST_SESSION_SIGNAL_CACHE_KEY)
        if bucket is None:
            bucket = {}
            st.session_state[_ST_SESSION_SIGNAL_CACHE_KEY] = bucket
        return bucket
    except Exception:
        return None


def invalidate_signal_history_cache(
    stock_code: str | None = None,
    *,
    trade_date: str | None = None,
) -> None:
    """新信号入库后失效缓存；``stock_code=None`` 时清空全部。"""
    if stock_code is None:
        _SIGNAL_HISTORY_CACHE.clear()
        bucket = _streamlit_signal_cache_bucket()
        if bucket is not None:
            bucket.clear()
        return
    key = _signal_history_cache_key(stock_code, trade_date)
    _SIGNAL_HISTORY_CACHE.pop(key, None)
    bucket = _streamlit_signal_cache_bucket()
    if bucket is not None:
        bucket.pop(key, None)


def fetch_signal_history_cached(
    stock_code: str,
    *,
    trade_date: str | None = None,
    ...
    """
    带 TTL 的当日信号历史读取：秒级轮询周期内无变更则内存/会话态降级返回，
    降低 ``fetch_signal_history_for_stock_on_date`` 对 SQLite 的读竞争。
    """
    key = _signal_history_cache_key(stock_code, trade_date)
    now = time.monotonic()
    ttl = _SIGNAL_HISTORY_CACHE_TTL_SEC

    if not force_refresh:
        st_bucket = _streamlit_signal_cache_bucket()
        ...
        cached = _SIGNAL_HISTORY_CACHE.get(key)
        if cached is not None:
            ts, rows = cached
            if (now - ts) < ttl:
                st_bucket = _streamlit_signal_cache_bucket()
                if st_bucket is not None:
                    st_bucket[key] = {"ts": ts, "rows": rows}
                return copy.deepcopy(rows)

    rows = fetch_signal_history_for_stock_on_date(stock_code, trade_date)
    _SIGNAL_HISTORY_CACHE[key] = (now, rows)
    st_bucket = _streamlit_signal_cache_bucket()
    if st_bucket is not None:
        st_bucket[key] = {"ts": now, "rows": rows}
    return copy.deepcopy(rows)
            ...
            panels.append(
                {
                    ...
                    "minute_df": None,
                    "signals_today": fetch_signal_history_cached(code),
                    "error": "empty",
                }
            )
            continue
