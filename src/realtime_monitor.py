...
def render_today_recommend_live_monitor_section() -> None:
    """
    「今日推荐」内嵌的 **临场实盘监控** 区：纵向大图 + 自动刷新（与历史三列区完全分离）。
    """
    ...
    st.subheader("⚡ 临场实盘监控")
    st.caption(
        "连续竞价时段每 30–60 秒自动刷新；单股自上而下：分时（九转）→ 成交量 → KDJ → 买点表。"
        "配色：上涨红、下跌绿。"
    )
    ...
    if in_session:
        try:
            ...
            st_autorefresh(interval=refresh_ms, key="top3_realtime_monitor_refresh")
        except ImportError:
            st.caption(
                "未安装 streamlit-autorefresh，请执行 `pip install streamlit-autorefresh` 启用 30–60 秒自动刷新。"
            )
    else:
        st.info("当前非连续竞价时段，暂停写入新信号；仍可查看今日已入库记录与最近分时（若接口有数据）。")

    targets = fetch_top3_selections_for_monitor()
    if not targets:
        st.warning("暂无 Top3 选股，无法启动实盘监控。")
        return
...
<div style="
  ...
  border-radius: 10px;
  padding: 14px 18px;
  margin: {'18px' if idx else '10px'} 0 14px 0;
...
">
  ...
  <span style="color:#475569;font-weight:600;margin-left:8px;">{name}</span>
...
""",
            ...
            if panel.get("error") == "empty":
                ...
                st.markdown("##### 今日信号记录")
                st.dataframe(
                    signals_to_display_dataframe(panel.get("signals_today")),
                    use_container_width=True,
                    hide_index=True,
                )
                continue

            fig = build_intraday_dashboard_figure(panel, height=540)
            if fig is not None:
