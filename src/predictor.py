...
def infer_ma_from_close_bias(
    ...
    *,
    ...
    """由原始乖离率倒推 20/60 日均线：``ma ≈ close / (1 + bias)``。"""
    c = float(close)
    d20 = float(bias_20) + 1.0
    d60 = float(bias_60) + 1.0
    ma20 = c / d20 if np.isfinite(c) and np.isfinite(d20) and abs(d20) > eps else float("nan")
    ma60 = c / d60 if np.isfinite(c) and np.isfinite(d60) and abs(d60) > eps else float("nan")
    return ma20, ma60


def is_major_trend_breakdown(
    close: float,
    ma20: float,
    ma60: float,
) -> bool:
    """
    大趋势破位硬风险：现价同时跌破 20/60 日均线，且 20 日线低于 60 日线（空头排列）。
    均线不可算时不触发（避免新股/停牌误杀）。
    """
    if not all(np.isfinite(x) for x in (close, ma20, ma60)):
        return False
...
def predict_universe_scores(
    ...
    *,
    ...
    out = feat_df[cols + list(FEATURE_COLUMNS)].copy()
    return as_of, out
...
def diagnose_single_stock(
    ...
    ma20_series = _roll_ewm_blend(df_hist["close"].astype(float), 20)
    ma60_series = _roll_ewm_blend(df_hist["close"].astype(float), 60)
    ma20_last = ma20_series.iloc[-1]
    ma60_last = ma60_series.iloc[-1]
    ma20_val = float(ma20_last) if pd.notna(ma20_last) else float("nan")
    ma60_val = float(ma60_last) if pd.notna(ma60_last) else float("nan")
    ...
    if (
        ...
        violated.append(
            f"中长期大趋势处于全面空头破位状态（现价 {close_price:.2f} 元同时低于20日生命线 "
            f"{ma20_val:.2f} 和60日牛熊线 {ma60_val:.2f}，且中短期均线全面死叉向下），缺乏上涨动量"
        )
