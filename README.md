...
- **仅使用日线 OHLCV**，无分时、无盘口
- T 日收盘确认信号并以 **收盘价** 作为买入价
- T+1 用 **最低价** 模拟 **-3%** 硬止损；开盘已在止损线下则按 T+1 收盘/开盘真实计提（防回测美化）
- 未触发止损则在 T+1 或 T+2 **收盘价** 平仓（可配置）
...
### 选股逻辑（`src/short_term/strategy.py`）

**硬性门槛（须全部满足）**

- 沪深300 环境分 ≥ `QUANT_SHORT_MIN_MARKET_SCORE`（默认 50）
- 收盘 > MA5 且 MA5 ≥ MA10，且 **收盘 > 开盘**（收阳）
- 剔除 ST、北交所（可关）、停牌、涨停日
- 板块近涨停：主板 ≥9.5%、创/科 ≥19.2%、北交所 ≥29.2%
- 5 日涨幅 ≤ 22%
- 流动性：成交额 ≥ 5000 万 **或** 换手 ≥ 2%
- 默认 **剔除** 创业板（300/301）、科创板（688），界面/CLI 可勾选纳入

**温和共振（4 项至少 3 项）**

1. 5 日量比 ≥ 1.25  
2. 1 日量比 ≥ 1.08（计算后 **上限 clip 10 倍**，防复牌畸变）  
3. DIF > DEA 且 MACD 柱 ≥ 前柱 × 0.8  
4. K > D，J ∈ [25, 90]，J 较前日升 ≥ 1.5  

**打分 `final_score`（排序）**

- 当日涨幅非线性 20%（2.5%~5.5% 满分，过高扣分）  
- 5 日量比 25%、MACD 柱改善 25%、J 斜率 15%  
- J ≥ 88 扣 30 分  

### 模拟执行（`src/short_term/execution.py`）

| 场景 | 平仓价 | `exit_reason` |
|------|--------|----------------|
| T+1 开盘 ≤ 止损线 | T+1 收盘（缺失则用开盘） | `t1_open_below_stop_limit` |
| T+1 盘中跌破 -3% | 买入价 × 0.97 | `t1_intraday_stop_loss` |
| 未触发止损 | T+1 或 T+2 收盘 | `t1_close_exit` / `t2_close_exit` |
...
| `QUANT_SHORT_MIN_MARKET_SCORE` | 大盘环境分下限 | 50 |
| `QUANT_SHORT_STOP_LOSS` | 止损比例 | 0.03 |
| `QUANT_SHORT_SELL_OFFSET` | 1=T+1 收盘卖，2=T+2 收盘卖 | 1 |
